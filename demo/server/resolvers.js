import { GraphQLError } from 'graphql';
import { getCompany } from './db/companies.js';
import { createJob, deleteJob, getJob, getJobs, getJobsByCompany, updateJob } from './db/jobs.js';
import { logger } from './logs.js'
import { apm } from './server.js'

export const resolvers = {
  Query: {
    company: async (_root, { id }) => {
      const transaction = apm.startTransaction('company-resolver', 'graphql');
      const company = await getCompany(id);
      if (!company) {
        const span = transaction.startSpan('No Company found');
        span.end();
        logger.error('No Company found with id ' + id);
        throw notFoundError('No Company found with id ' + id);
      }
      transaction.end('success');
      logger.info('Company found with id ' + id);
      return company;
    },
    job: async (_root, { id }) => {
      const job = await getJob(id);
      if (!job) {
        logger.error('No Job found with id ' + id);
        throw notFoundError('No Job found with id ' + id);
      }
      logger.info('Job found with id ' + id);
      return job;
    },
    jobs: () => getJobs(),
  },

  Mutation: {
    createJob: (_root, { input: { title, description } }, { user }) => {
      const transaction = apm.startTransaction('createJob-resolver', 'graphql');
      if (!user) {
        const span = transaction.startSpan('Missing authentication');
        span.end();
        logger.error('Missing authentication');
        throw unauthorizedError('Missing authentication');
      }
      transaction.end('success');
      return createJob({ companyId: user.companyId, title, description });
    },

    deleteJob: async (_root, { id }, { user }) => {
      const transaction = apm.startTransaction('deleteJob-resolver', 'graphql');
      if (!user) {
        const span = transaction.startSpan('Missing authentication');
        span.end();
        logger.error('Missing authentication');
        throw unauthorizedError('Missing authentication');
      }
      const job = await deleteJob(id, user.companyId);
      if (!job) {
        const span = transaction.startSpan('No Job found!');
        span.end();
        logger.error('No Job found with id ' + id);
        throw notFoundError('No Job found with id ' + id);
      }
      logger.info('Job found with id ' + id);
      transaction.end('success');
      return job;
    },

    updateJob: async (_root, { input: { id, title, description } }, { user }) => {
      const transaction = apm.startTransaction('updateJob-resolver', 'graphql');
      if (!user) {
        const span = transaction.startSpan('Missing authentication');
        span.end();
        logger.error('Missing authentication');
        throw unauthorizedError('Missing authentication');
      }
      const job = await updateJob({ id, companyId: user.companyId, title, description });
      if (!job) {
        const span = transaction.startSpan('No Job found');
        span.end();
        logger.error('No Job found with id ' + id);
        throw notFoundError('No Job found with id ' + id);
      }
      logger.info('Job found with id ' + id);
      transaction.end('success');
      return job;
    },
  },

  Company: {
    jobs: (company) => getJobsByCompany(company.id),
  },

  Job: {
    company: (job) => getCompany(job.companyId),
    date: (job) => toIsoDate(job.createdAt),
  },
};

function notFoundError(message) {
  return new GraphQLError(message, {
    extensions: { code: 'NOT_FOUND' },
  });
}

function unauthorizedError(message) {
  return new GraphQLError(message, {
    extensions: { code: 'UNAUTHORIZED' },
  });
}

function toIsoDate(value) {
  return value.slice(0, 'yyyy-mm-dd'.length);
}
